...
/* ── Main ───────────────────────────────────────────── */
export async function generatePdf({ data, rr, colleges, logoSrc }) {
  ...
  font("bold", 17);
  txt("#1e3a8a");
  text("ACME MCA Entrance Academy", margin + 62, y + 18);

  font("normal", 9);
  txt("#64748b");
  text("OFFICIAL NIMCET RANK PREDICTION REPORT", margin + 62, y + 33);

  // Date right-aligned
  font("bold", 9);
  txt("#64748b");
  text("Date:", PW - margin, y + 18, { align: "right" });
  font("bold", 11);
  txt("#0f172a");
  text(date, PW - margin, y + 32, { align: "right" });
  font("normal", 8);
  txt("#94a3b8");
  text("Ref: " + data.regNo, PW - margin, y + 46, { align: "right" });
  ...
  font("normal", 8); txt("#94a3b8");
  text(
    `(c) ${new Date().getFullYear()} ACME MCA Entrance Academy  |  acmea.courses.store`,
    PW / 2, y, { align:"center" }
  );
  text("Generated by ACME NIMCET Rank Predictor Tool", PW / 2, y + 13, { align:"center" });
